    ...
    const fetchCategory = async () => {
      try {
        const res = await api.get(
          ...
          }/categoryarticle/getcategorybyid/${id}`,
          {
            headers: {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const res = await api.put(
        `${
          import.meta.env.VITE_APP_BASE_URL
        }/categoryarticle/updatecategory/${id}`,
        formDataToSend,
        {
          headers: {
            Authorization: `Bearer ${secureLocalStorage.getItem(
      ...
      if (res.data.success) {
        ...
        toast.error(res.data.message || "Failed to update Category Article.");
      }
    } catch (error) {
      console.error("Error updating Category Articles:", error.message);
      toast.error("Failed to update Category Articles.");
    }
  };
              ...
              <div className="full graph_head">
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="row pr-[10px]">
                    <button
                      type="submit"
                      className="ml-auto w-fit bg-blue-500 px-4 py-3 rounded-md text-white my-4"
                    >
                      Update Category
                    </button>
                  </div>
